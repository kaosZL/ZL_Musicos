...
**📱 手机扫码导入** — 电视端出二维码，手机同网扫码：粘音源链接/脚本、导各平台歌单、改名删除导出，告别遥控器输 URL

**❤️ 歌单全家桶** — 链接/歌名清单/洛雪文件一键导入，收藏、加歌、长按管理、手机导出备份一站齐
...
| **长按 OK**（播放列表） | 呼出歌曲菜单：加入我的歌单 / ❤ 收藏 / 从列表删除 |
| **长按 OK**（我的歌单内） | 弹窗确认从歌单删除该歌曲 |
| **长按 OK**（歌单卡片） | 歌单管理（电视改名 / 删除） |
| **长按 OK**（搜索历史标签） | 删除该条搜索历史 |
| OK（播放页·控制栏未呼出） | 暂停 / 继续播放 |
| Back | 返回上一页 |
| Play/Pause | 播放页暂停或继续 |
...
| 左 / 右（播放页·短按） | 上一首 / 下一首 |
| 左 / 右（播放页·长按） | 连续快进 / 快退（前 4 次 ±5s，之后升级 ±30s） |
...
| 场景 | 操作 |
| --- | --- |
| 导入歌单 | 设置 → 手机扫码导入 → 手机页粘贴歌单链接 / 「歌名 - 歌手」清单 / 上传洛雪文件（.json/.lxmc/压缩包） |
| 收藏歌曲 | 播放列表长按 OK → ❤ 我的收藏 |
| 加入歌单 | 播放列表长按 OK 或「全部加入歌单」→ 选目标歌单（按钮带歌曲数，默认加到最前，可切换） |
| 删除歌曲 | 我的歌单 → 打开歌单 → 长按 OK 确认删除 |
| 改名 / 删除歌单 | 电视：我的歌单长按卡片；手机页：读取歌单后操作（支持中文改名） |
| 导出备份 | 手机页「读取歌单」→ 导出 → 下载洛雪格式 .json |
| 定时关闭 | 设置 → 定时关闭 → 播完即停 / 30 / 60 / 90 分钟 |
| 搜索历史 | 自动记录最近 8 条；长按删单条，一键清空 |
...
- 弹窗焦点可视化：所有弹窗按钮焦点白底白边光晕高亮，遥控左右切换清晰可见（v1.9.3 修复长按 OK / 清空列表弹窗中焦点不可见的问题）。
- 播放页遥控直控：控制栏未呼出时 OK＝暂停/继续、左右短按切歌、长按连续快进快退（±5s），对齐 Apple Music TV 交互。
- 焦点系统增强：列表与按钮焦点高亮强化（描边+光晕+缩放），内容首行按上直达顶部导航栏。
- 搜索/排行榜音源筛选：新增酷狗/酷我/网易/QQ/咪咕子源切换 Tab。
- 歌单导入（手机扫码页）：支持网易云/QQ音乐/酷狗/酷我/咪咕歌单链接、「歌名 - 歌手」清单（逐首用现有音源搜索匹配，未匹配点名告知）、洛雪导出文件（.json/.lxmc）及压缩包/.crx 内的歌单文件；中文全程无损，歌单名可在手机端改（电视无法输中文）。
- 歌单管理：「我的歌单」卡片带歌曲数与首图封面；播放列表长按加入歌单（选择器带数量，默认加到最前可切换，已存在歌曲如实提示）；「全部加入歌单」一键导入；歌单内长按删除歌曲。
- 我的收藏：歌单页常驻 ❤ 收藏卡片 + 加歌选择器第一项，与自建歌单同权管理。
- 歌单导出：手机页一键导出洛雪格式 .json，备份/迁移不丢数据。
- 定时关闭（睡眠模式）：播完当前这首就停，或 30/60/90 分钟后自动暂停。
- 在线歌单/排行榜分页加载：滚到底「加载更多」拉取全量（自动去重），加载失败可一键重试。
- 搜索历史：最近 8 条持久化保存（重启不丢），长按删单条、一键清空。
- 播放器长按快进：前 4 次 ±5s 精调，之后自动升级 ±30s 大步长。
