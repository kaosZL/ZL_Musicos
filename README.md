...
ZL-Music TV 是基于 [lx-music-mobile 1.8.4](https://github.com/lyswhut/lx-music-mobile) 二次开发的 Android TV 音乐播放器。当前版本（v1.9.3）重点建设了 TV 端体验：手机扫码导入音源、完整音源管理、拼音搜索联想、全分辨率等比适配与遥控器优先操作；v1.9.x 系列补齐弹窗焦点可视化、播放页遥控直控（OK＝暂停/播放、左右短按切歌、长按快进快退）与首行直达顶部导航。
...
```text
android/app/build/outputs/apk/release/zl-music-v1.9.3-arm64-v8a.apk
android/app/build/outputs/apk/release/zl-music-v1.9.3-x86_64.apk
android/app/build/outputs/apk/release/zl-music-v1.9.3-universal.apk
```
...
adb -s 127.0.0.1:62001 install -r android/app/build/outputs/apk/release/zl-music-v1.9.3-x86_64.apk
```
...
- `zl-music-v1.9.3-arm64-v8a.apk`：大多数 64 位 Android TV / 盒子。
- `zl-music-v1.9.3-universal.apk`：不确定设备架构时使用。
- `zl-music-v1.9.3-x86_64.apk`：模拟器。
