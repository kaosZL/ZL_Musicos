...
export const onTVRemoteEvent = (handler: (event: TVRemoteEvent) => void): () => void => {
  // eslint-disable-next-line @typescript-eslint/no-unsafe-argument
  const eventEmitter = new NativeEventEmitter(UtilsModule)
  const eventListener = eventEmitter.addListener('tv-remote-event', event => {
    handler(event as TVRemoteEvent)
  })
