...
import { ensureTVPresetUserApi } from '@/core/tvPresetUserApi'

let isFirstPush = true
const handlePushedHomeScreen = async() => {
  const isTVMode = getCachedIsTV()

  if (!isTVMode) await cheatTip()
  if (isTVMode && !settingState.setting['common.isAgreePact']) {
...
export default async() => {
  ...
  if (getCachedIsTV()) {
    await ensureTVPresetUserApi()
    bootLog('TV preset User Api ensured.')
  } else {
