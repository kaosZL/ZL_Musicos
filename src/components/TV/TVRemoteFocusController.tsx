...
const TVRemoteFocusController = ({ componentId }: { componentId: string }) => {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onTVRemoteEvent(({ eventType, eventKeyAction }) => {
      ...
      // DOWN=记录 & 无长按目标时立即短按 / REPEAT=长按 / UP=无 repeat 时短按
      if (eventType === 'select') {
        ...
        } else if (eventKeyAction === KEY_ACTION_REPEAT && !selectHoldRef.current) {
          selectHoldRef.current = true
          longPressActiveTVTarget()
        } else if (eventKeyAction === KEY_ACTION_UP && !selectHoldRef.current) {
          // 有长按能力的按钮：松开时才确定是短按（避免 300ms 定时器抢先触发）
          pressActiveTVTarget()
        }
        return
      }
